...
function ViewAllFolder({ searchQuery, onFolderSelect }) {
  ...
  console.log("these are folders", folders);
    ...
    <div className='p-6'>
      <div className=''>
        <div className='flex justify-between pr-4'>
          <p className='text-2xl font-normal text-[#1F1F1F] mb-4'>All Folders</p>
          ...
          {filteredFolders.map((folder, index) => (
            ...
            <div key={folder.id} className='flex justify-between p-3 border-2 border-gray-200 rounded-lg relative mb-2'>
              <div className=''>
                ...
                >
                  {folder.name}
                  <span className="text-gray-500 ml-2 text-sm">({folder.fileCount} files)</span>
                </span>
                {editable === folder.id && (
                  <div 
                ...
                <div className="text-xs text-gray-600 mt-2 flex flex-col gap-y-2 ">
                  <span className="font-medium text-sm  text-black">Created: <span className="text-gray-500 font-normal text-sm">{folder.created ? new Date(folder.created).toLocaleString() : 'Undefined'}</span></span>
                  <span className="font-medium text-sm  text-black">Updated: <span className="text-gray-500  font-normal text-sm">{folder.updated ? new Date(folder.updated).toLocaleString() : 'Undefined'}</span></span>
                </div>
