...
const Dashboard = ({ folderId, onFolderSelect, searchQuery }) => {
  ...
  if (files?.subfolders || files?.files) {
    ...
    // Add `type` to distinguish in render
    ...
    // Merge with subfolders first
    sortedFiles = [...mappedSubfolders, ...mappedFiles];
    // console.log("Sorted files with subfolders first:", sortedFiles);
  }
  ...
  const filteredMobileFiles = (files.files || []).filter((file) =>
            ...
            <div className="flex gap-5">
              ...
              <div>
                <button
                  onClick={() => {
                    addFolder();
                    // handleAddParentFolder(folderId);
                        ...
                        file.access_details
                          ...
                          .map((detail, index) => (
                            <img
                              ...
                              className={`absolute shadow-lg h-8 w-8 rounded-full ${
                                index === 0
                                  ? "z-20 right-6"
                                  : index === 1
                                  ? "z-10 right-10"
  ...
  {Array.isArray(sortedFiles) && sortedFiles.length > 0 && (
    sortedFiles.map((item) => {
      if (item.type === "subfolder") {
        return (
          <tr key={item._id} className="border-b-2 text-xs sm:text-sm">
      ...
      {isUploading && (
        <div className="fixed inset-0 w-screen max-h-screen bg-gray-800 bg-opacity-50 flex items-center justify-center z-50 ">
          <div className="bg-white p-6 rounded shadow-lg max-w-screen">
            ...
            <div
              className={`items-center mb-4 text-center ${
                folderId === 0 || folderId === "0" ? "mt-4" : ""
              }`}
            >
              <h2 className="text-lg font-semibold">Upload File</h2>
            </div>
            <div
              ...
              className={`border-dashed border-2 p-4 text-center ${
                isDragOver ? "border-blue-700 bg-blue-50" : "border-blue-500"
              }`}
            >
                    ...
                    {filteredDesignees.map((designee) => (
                      <div
                        ...
                        className={`p-2 cursor-pointer hover:bg-gray-100 ${
                          selectedEmails.includes(designee.email)
