...
router.post("/create-folder", authenticateToken, async (req, res) => {
  ...
  console.log("Decoded user_id from token:", req.user);  
...
router.get("/get-folders", authenticateToken, async (req, res) => {
  try {
    ...
    const subfolderCountMap = new Map(subfolderCounts.map(sc => [sc._id ? sc._id.toString() : null, sc.subfolderCount]));
    ...
    const decryptedFolders = folders.map(folder => {
      ...
      return {
        ...
        subfolder_count: subfolderCountMap.get(folder._id.toString()) || 0
      };
    });
...
router.post("/get-files", authenticateToken, async (req, res) => {
  try {
    ...
    const decryptedSubfolders = subfolders.map((subfolder) => {
      return {
        _id: subfolder._id,
        folder_name: decryptField(subfolder.folder_name, subfolder.iv_folder_name),
        aws_folder_link: decryptField(subfolder.aws_folder_link, subfolder.iv_folder_link),
